/*
    ...
    por borbulhamento, valores menores gradualmente  sobem para a parte superior do
    array como bolhas de ar subindo na água, enquanto as bolhas maiores afundam.
    A classificação por  borbulhamento faz várias passagens pelo array. Em cada passagem,
    sucessivos pares de elementos são comparados. Se um par estiver  na ordem crescente
    (ou os valores forem idênticos), deixamos os valores como eles estão. Se um par estiver
    na ordem decrescente, seus  valores são trocados no array. Escreva um programa que
    classifica um array de 10 inteiros utilizando classificação por borbulhamento.
...
void adicionar(int vetor[], int tamanho)
{
    ...
    int aleatorio =  1 + rand() % 10; // gera o primeiro aleatório
    ...
    while( true )
    {
        ...
        // se não for
        aleatorio =  1 + rand() % 10; // gera número aleatório
